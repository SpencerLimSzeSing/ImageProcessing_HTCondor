...
<img src="docs/Instances_AWS.png" alt="Alt text" width="550" height="150">  
...
<img src="docs/Architecture_AWS.png" alt="Alt text" width="350" height="300"> 
...
### 1. DAGMan Workflow Submission
    ...
    The workflow was submitted using HTCondor DAGMan, which is responsible for managing job dependencies and execution order. DAGMan interprets the DAG file, submits individual jobs to HTCondor, and monitors their execution status.

    During submission, DAGMan automatically generated its own log and status files to track workflow progress and completion. Once submitted, the workflow execution could be monitored using standard HTCondor commands such as condor_q and condor_history.

### 2. Job-Level Validation Strategy

    To ensure correct workflow execution, each job was first validated independently before being integrated into the DAG. This approach helped verify that:
    ...
    By validating the jobs individually, potential issues related to job configuration were isolated from workflow orchestration concerns.

### 3. Debugging and Root Cause Analysis

    Debugging was conducted using standard HTCondor and HPC troubleshooting practices, including:
    ...
    - Ensuring absolute paths were used for executables, logs, and output files
...
### 4. Execution Environment 
    ...
    HTCondor jobs execute on remote execution nodes, not on the submission node.
    ...
    Misalignment between submission and execution environments was identified as a key factor that can lead to job execution failures if not properly managed.
...
6. HPC execution environment awareness
7. Technical documentation

**Future Improvements**
1. Automate environment validation before job submission
2. Add retry and failure-handling policies in DAGMan
3. Integrate monitoring dashboards for job execution
4. Extend workflow to larger datasets or parallel branches